  ...
  try {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        status: "pending",
        items: {
          create: items.map((item: { productId: string; slug: string; title: string; price: number; quantity: number; image?: string }) => ({
    ...
    if (!razorpay) {
      return NextResponse.json({
        orderId: order.id,
        directSuccess: true,
        url: `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/order/success?orderId=${order.id}`,
      });
    }
