...
export default function HomePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [productsRes, catsRes, collsRes, slidesRes, settingsRes] = await Promise.all([
        ...
        if (collsRes.ok) {
          ...
          if (collsData?.length) setCollections(collsData);
        }
        if (slidesRes.ok) {
  ...
  return (
    <main>
      {/* Hero Section */}
      {heroSlides.length > 0 ? (
        <EmblaCarousel opts={{ loop: true, align: "start" }} className="relative">
          <CarouselContent>
        ...
        <section className="py-20 px-6 bg-[#F8F5F0]">
          <div className="max-w-7xl mx-auto">
            ...
            <StaggerContainer className="grid md:grid-cols-2 gap-6" staggerDelay={0.1}>
              {collections.slice(0, 4).map((collection: Collection, index: number) => (
                <StaggerItem key={collection.id}>
                  <ScaleIn delay={index * 0.1}>
                    <Link href={`/collections/${collection.slug}`} className="group block">
                      <div className={`relative overflow-hidden rounded-sm ${index === 0 || index === 3 ? "aspect-[16/9]" : "aspect-[4/3]"}`}>
                        ...
                        {collection.image ? (
                          ...
                          <div className="w-full h-full bg-[#E8DDD0] flex items-center justify-center">
                            <span className="text-[#C9A84C]/30 text-8xl font-serif">✦</span>
                          </div>
                        )}
